...
export type ProductTileProps = {
  ...
  href?: string;
  /** Fallback click handler when there is no href (e.g. the portal builder). */
  onOpen?: () => void;
...
export function ProductTile({
  ...
  return (
    ...
    >
      {href ? (
        <>
          {/* Image links to the same target but stays out of the tab order — the
              title link below is the single, named focus stop for the card. */}
          <Link href={href} className="pds-product__stage" tabIndex={-1} aria-hidden>
            {stageInner}
          </Link>
          <Link href={href} className="pds-product__body">
            {bodyInner}
          </Link>
        </>
      ) : (
