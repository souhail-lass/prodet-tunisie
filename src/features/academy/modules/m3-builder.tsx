...
const PRODUCTS: Product[] = [
  {
    ...
    ph: '10–11 (alcalin)',
    foam: 2,
    soil: 'Grasse + générale',
    base: [
      { name: 'Eau adoucie', role: 'Base', pct: 62, col: '#41AAE2' },
      {
        name: 'Acide sulfonique (LAS)',
        role: 'Tensioactif anionique — lave',
        pct: 7,
        col: '#095296',
      },
      { name: 'Tensioactif non-ionique', role: 'Dégraisse à basse temp.', pct: 4, col: '#095296' },
      {
        name: 'Métasilicate + carbonate',
        role: 'Builders alcalins — pH lavant',
        pct: 7,
        col: '#24883E',
      },
  ...
  },
  {
    key: 'degraissant',
    ico: '🍳',
    label: 'Dégraissant cuisine',
    ph: '12–13 (très alcalin)',
    foam: 1,
    soil: 'Grasse lourde',
    base: [
  ...
  },
  {
    key: 'detartrant',
    ico: '🚽',
    label: 'Détartrant sanitaire',
    ph: '1–2 (acide fort)',
    foam: 1,
    soil: 'Minérale / calcaire',
    base: [
      { name: 'Eau', role: 'Base', pct: 78, col: '#41AAE2' },
      {
        name: 'Acide (chlorhydrique/phosphorique)',
        role: 'Détartre — dissout le calcaire',
        pct: 15,
        col: '#C0392B',
      },
  ...
  },
  {
    ...
    base: [
      ...
      {
        name: 'Base cationique (Dequart)',
        role: 'Adoucit — se fixe sur fibres',
        pct: 12,
        col: '#8E44AD',
      },
...
export function FormulaBuilder() {
  ...
  if (hard && p.key !== 'detartrant' && p.key !== 'assouplissant') {
    rows.splice(1, 0, {
      name: 'EDTA (séquestrant)',
      role: 'Capture le calcaire — Tunisie',
      pct: 3,
      col: '#24883E',
      extra: true,
    });
  ...
  if (color && p.key === 'lessive') {
    rows.push({
      name: 'Booster oxygéné (percarbonate) — poudre séparée',
      role: 'Taches vin, café, thé',
      pct: 0,
      col: '#2980B9',
      extra: true,
    });
  ...
  return (
    <div className="academy-lab">
      <span className="academy-lab__badge">
        <Zap size={13} aria-hidden /> Constructeur interactif
      </span>
      ...
      <div className="academy-profile">
        <div className="academy-prof">
          <div className="pk">pH</div>
          <div className="pv">{p.ph}</div>
        </div>
        <div className="academy-prof">
          <div className="pk">Mousse</div>
          <div className="pv">{FOAM_LABELS[p.foam]}</div>
        </div>
        <div className="academy-prof">
          <div className="pk">Cible</div>
          <div className="pv">{p.soil}</div>
        </div>
      </div>
    </div>
  );
