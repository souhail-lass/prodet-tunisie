...
export function MicelleLab() {
  ...
  return (
    <div className="academy-lab">
      ...
      <p className="academy-lab__sub">
        Glisse pour ajouter des tensioactifs. Queues dans la graisse, têtes dans l’eau — la saleté
        se détache.
      </p>
      <div className="academy-stage">
        ...
        >
          ...
          <text x={cx} y={cy + 5} textAnchor="middle" fontSize="12" fontWeight="600" fill="#7A5C12">
            graisse
          </text>
          {surfactants.map((s, i) => (
            <g key={i}>
      ...
      <div className="academy-concept green" style={{ margin: 0 }}>
        ...
        <p>
          Quand assez de tensioactifs entourent la graisse (une <strong>micelle</strong>), elle se
          détache. C’est tout le principe — d’où l’intérêt de ne pas trop réduire le taux, comme sur
          le Prolax.
        </p>
      </div>
    </div>
...
const TYPES = [
  {
    ...
    chargeCol: '#C0392B',
    nm: 'Anionique',
    d: 'Le plus courant et le moins cher. Lave et mousse bien. Acide sulfonique, texapon, NANSA. Sensible au calcaire (d’où l’EDTA).',
    ex: ['Acide sulfonique', 'Texapon', 'NANSA'],
    foam: 4,
  },
  {
    charge: '○',
    chargeCol: '#6B6B67',
    nm: 'Non-ionique',
    d: 'Le meilleur dégraissant, surtout à chaud. Mousse peu — idéal lessive machine. NP9 (à remplacer) ou alcool gras éthoxylé.',
    ex: ['NP9', 'Alcool éthoxylé'],
    foam: 1,
  },
  {
    charge: '+',
    chargeCol: '#2980B9',
    nm: 'Cationique',
    d: 'Ne lave pas — se fixe sur les fibres pour adoucir. Base assouplissant (Dequart). Aussi désinfectant. Incompatible avec les anioniques.',
    ex: ['Base assouplissant', 'Ammonium quaternaire'],
    foam: 1,
  },
  {
    charge: '±',
    chargeCol: '#8E44AD',
    nm: 'Amphotère',
    d: 'Doux, s’adapte au pH. Améliore la mousse et adoucit l’agressivité. Bétaïne (Saterin). Savon mains et vaisselle.',
    ex: ['Bétaïne', 'Saterin'],
    foam: 2,
  },
];
  ...
  return (
    <div className="academy-lab">
      <div className="academy-typetabs">
        {TYPES.map((ty, idx) => (
          <button
            ...
            aria-pressed={idx === i}
          >
            <div className="charge" style={{ color: ty.chargeCol }}>
              {ty.charge}
            </div>
      ...
      <div className="academy-typedetail">
        ...
        <p>{t.d}</p>
        {t.ex.map((e) => (
          <span className="academy-ex" key={e}>
            {e}
          </span>
        ))}
...
export function EdtaLab() {
  ...
  return (
    <div className="academy-lab">
      ...
      <p className="academy-lab__sub">
        Sans EDTA, le calcium (rouge) neutralise. Ajoute de l’EDTA, vois-les se libérer (vert).
      </p>
            ...
            return (
              <g key={i}>
                <circle
                  ...
                  cy={y}
                  r="11"
                  fill={free ? '#E8F5EE' : '#FCEBEB'}
                  stroke={col}
                  strokeWidth="2"
                />
                {free ? (
                  <path
                    d={`M${x - 4} ${y - 4} L${x} ${y} L${x + 4} ${y - 4} M${x} ${y} L${x} ${y + 5}`}
                    stroke={col}
                    strokeWidth="2"
                    fill="none"
                  />
      ...
      <div className="academy-readout">
        <div className="academy-metric">
          <div className="v" style={{ color: 'var(--ac-green)' }}>
            {eff}%
          </div>
          <div className="l">Efficacité</div>
        </div>
        <div className="academy-metric">
          <div className="v" style={{ color: '#C0392B' }}>
            {100 - eff}%
          </div>
          <div className="l">Bloqués</div>
        </div>
      </div>
      <div className="academy-concept green" style={{ margin: '14px 0 0' }}>
        <div className="academy-concept__label">La leçon Prodet</div>
        <p>
          Les premiers % d’EDTA donnent le plus gros gain, puis ça plafonne. 2-3% suffisent dans le
          Prolax. Un peu de séquestrant pas cher rend tes tensioactifs chers bien plus efficaces.
        </p>
      </div>
    </div>
  );
...
const PH_ZONES = [
  {
    ...
    z: 'Acide fort',
    c: '#C0392B',
    t: 'Détartrage puissant, détruit calcaire et rouille. Pilax, Profon. Corrosif.',
  },
  {
    max: 6,
    z: 'Acide doux',
    c: '#E67E22',
    t: 'Détartrage léger, rinçage, brillance. Prolac, liquide de rinçage. pH des assouplissants (4-5).',
  },
  {
    max: 8,
    z: 'Neutre',
    c: '#2ECC71',
    t: 'Doux pour peau et surfaces délicates. Savons mains. Ne dégraisse pas fort.',
  },
  {
    max: 11,
    z: 'Alcalin',
    c: '#2980B9',
    t: 'Dégraissage efficace, saponifie. Tes lessives (Prolax ~10-11).',
  },
  {
    max: 14,
    z: 'Alcalin fort',
    c: '#6C3483',
    t: 'Dégraissage extrême, graisses cuites. Profour. Attaque l’aluminium.',
  },
];
const PH_PRODUCTS = [
  ...
  return (
    <div className="academy-lab">
      <div className="academy-phprods">
        {PH_PRODUCTS.map((p) => (
          <span className="academy-phprod" style={{ left: p.left }} key={p.label}>
            {p.label}
          </span>
        ))}
      </div>
      ...
      <div className="academy-typedetail" style={{ marginTop: 8, minHeight: 0 }}>
        <span className="academy-zonepill" style={{ background: `${zone.c}22`, color: zone.c }}>
          {zone.z}
        </span>
...
const BLEACH = {
  cl: {
    Puissance: 'Très fort, rapide',
    Couleur: '❌ Blanc uniquement',
    Fibres: 'Agressif — use les fibres',
    Coût: 'Très bon marché',
    Taches: 'Détruit tout (blanc)',
    Sécurité: 'Odeur forte, danger avec acide',
  },
  ox: {
    Puissance: 'Fort, progressif',
    Couleur: '✓ Couleur ET blanc',
    Fibres: 'Doux pour les fibres',
    Coût: 'Moyen',
    Taches: 'Vin, café, thé, fruits',
    Sécurité: 'Sûr, sans odeur agressive',
  },
};
...
export function SaltLab() {
  ...
  return (
    <div className="academy-lab">
      ...
      <p className="academy-lab__sub">
        Le sel épaissit les formules au SLES (texapon)… mais trop de sel et ça redevient liquide.
      </p>
      <svg
        viewBox="0 0 600 280"
        preserveAspectRatio="xMidYMid meet"
      ...
      >
        ...
        <text x="310" y="270" textAnchor="middle" fontSize="12" fill="#6B6B67">
          Sel ajouté →
        </text>
        <text
          x="20"
        ...
        {p >= 45 && p <= 65 ? (
          <text x="310" y="40" textAnchor="middle" fontSize="11" fontWeight="600" fill="#24883E">
            point optimal
          </text>
        ) : null}
      ...
      <div className="academy-readout">
        <div className="academy-metric">
          <div className="v" style={{ color: col, fontSize: 18 }}>
            {vis}
          </div>
          <div className="l">Viscosité</div>
        </div>
        <div className="academy-metric">
          <div className="v" style={{ color: col, fontSize: 15 }}>
            {zone}
          </div>
          <div className="l">État</div>
        </div>
      </div>
      <div className="academy-concept amber" style={{ margin: '14px 0 0' }}>
        <div className="academy-concept__label">Le sur-salage</div>
        <p>
          « J’ajoute du sel et d’un coup ça reliquéfie ! » → c’est la partie descendante. Ajoute le
          sel progressivement, arrête-toi au sommet. Et le sel n’épaissit QUE le SLES — sinon, amide
          de coco.
        </p>
      </div>
    </div>
