 ...
 * repeating them.
 */
export const requireClientPortalAccess = cache(async (): Promise<ClientPortalAccess> => {
  let supabase;
  try {
  ...
  if (!membershipRow) {
    const [row] = await db
      ...
      .innerJoin(schema.customer, eq(schema.userCustomer.customerId, schema.customer.id))
      .where(and(eq(schema.userCustomer.userId, appUser.id), isNull(schema.customer.deletedAt)))
      .limit(1);
